...
export default function NewItemPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert('Resim yükleme hatası. Lütfen tekrar deneyin.')
    } finally {
      setUploading(false)
    }
  }
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setFormData(prev => ({
            ...
            sodium: data.sodium || prev.sodium,
            ingredients: data.ingredients || prev.ingredients,
            allergens: data.allergens ? data.allergens.join(', ') : prev.allergens,
            dietary_info: data.dietary_info ? data.dietary_info.join(', ') : prev.dietary_info,
        ...
        if (error.errorCode === 'AI_OVERLOADED') {
          alert('AI servisi şu anda yoğun. Lütfen 10-20 saniye bekleyip tekrar deneyin. Sistem otomatik olarak tekrar deneyecektir.')
        } else if (error.errorCode === 'RATE_LIMITED') {
          alert('Çok fazla istek gönderildi. Lütfen 30 saniye bekleyip tekrar deneyin.')
        } else {
          alert(`AI zenginleştirme başarısız: ${error.error || 'Bilinmeyen hata'}`)
        }
      }
    } catch (error) {
      console.error('AI enrich error:', error)
      alert('AI zenginleştirme sırasında bir hata oluştu. Lütfen tekrar deneyin.')
    } finally {
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={!formData.name || isAiEnriching}
                  className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center gap-2 relative overflow-hidden"
                >
                  {isAiEnriching && (
                    <div
                  ...
                  <span className="relative z-10 flex items-center gap-2">
                    <Sparkles size={16} className={isAiEnriching ? 'animate-spin' : ''} />
                    {isAiEnriching ? `${Math.round(aiProgress)}%` : 'AI ile Doldur'}
                  </span>
                </button>
              </div>
              {isAiEnriching && (
            ...
            <div>
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  placeholder="https://example.com/image.jpg veya aşağıdan dosya yükleyin"
                />
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                    placeholder="Ürünün detaylı içerik ve malzeme açıklaması. Örn: Günlük taze malzemelerle hazırlanan cappuccino, %100 Arabica kahve çekirdekleri, organik süt ve el yapımı süt köpüğü ile servis edilir. Katkı maddesi içermez."
                  />
