...
export default function SliderManagement({ venueSlug }: SliderManagementProps) {
  ...
  const fetchSliders = async () => {
    try {
      const response = await fetch(`/api/user/venue/${venueSlug}/sliders`, {
        credentials: 'include'
      })

      console.log('🎨 Slider fetch response status:', response.status)
      ...
      if (response.ok) {
        ...
        console.error('❌ Slider fetch failed:', response.status)
      }
    } catch (error) {
  ...
  const handleSaveSlider = async (sliderId: number) => {
    ...
    setSaving(sliderId)
    try {
      const response = await fetch(`/api/user/venue/${venueSlug}/sliders/${sliderId}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
  ...
  const handleMoveSlider = async (sliderId: number, direction: 'up' | 'down') => {
    ...
    try {
      const response = await fetch(`/api/user/venue/${venueSlug}/sliders/reorder`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
